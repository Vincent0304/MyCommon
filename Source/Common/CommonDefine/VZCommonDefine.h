...
//#define DEBUG_MODE
#define VZSYSTEM_FONT_WORD NSLocalizedString(@"FZSEK--GBK1-0",nil)
#define VZSYSTEM_FONT_NUMBER @"FZSEK--GBK1-0"
#define VZSYSTEM_FONT_LETTER @"FZSEK--GBK1-0"




#define VZRADIANS_TO_DEGREES(radians) ((radians) * (180.0 / M_PI))

#define VZDEGREES_TO_RADIANS(angle) ((angle) / 180.0 * M_PI)

#define VZSYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(v)  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] != NSOrderedAscending)

#define VZIS_POWER_OF_TWO(x) ((x != 0) && ((x & (~x + 1)) == x))
