...
+ (NSData*)encryptAESDataFloat:(float)value
{
...
+ (float)decryptAESDataFloat:(NSData*)data
{
    float score = 0;
    NSData *decryData = [data AES256DecryptWithKey:APP_PUBLIC_PASSWORD];
    NSString* string = [[NSString alloc] initWithData:decryData encoding:NSUTF8StringEncoding];
    score = [string floatValue];
    return score;
}

+ (NSData*)encryptAESDataInt:(int)value
{
...
+ (int)decryptAESDataInt:(NSData*)data
{
    int score = 0;
    NSData *decryData = [data AES256DecryptWithKey:APP_PUBLIC_PASSWORD];
    NSString* string = [[NSString alloc] initWithData:decryData encoding:NSUTF8StringEncoding];
    score = [string intValue];
    return score;
}
