...
VZ_SYNTHESIZE_SINGLETON_FOR_CLASS(VZShareManager)
...
-(void) shareWithFile:(NSString*)filename
{
    ...
    if([self isSocialAvailable])
    {
        ...
        NSString* shareContent = [[VZIdentifyManager sharedVZIdentifyManager] objectForIdentifyInfoDictionaryKey:kVZIdentifyShareContent];
        NSString* downloadLink = [[VZIdentifyManager sharedVZIdentifyManager] objectForIdentifyInfoDictionaryKey:kVZIdentifyDownloadLink];
        ...
        NSArray* activity = @[[[WeixinTimelineActivity alloc] init], [[WeixinSessionActivity alloc] init]];
        
        VZShareViewController *activityController =
        
        [[VZShareViewController alloc] initWithActivityItems:activityItems  applicationActivities:activity];
...
-(void) shareWithScreenShot;
{
    ...
    if([self isSocialAvailable])
    {
        ...
        if(VZSYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0"))
        {
